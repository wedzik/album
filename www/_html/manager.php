...
class Manager {
    ...
    private function _dir_info($dir, $initial=TRUE) {
        ...
        if ((DIR_IMAGE_FILE) and file_exists(GALLERY_ROOT.$dir.DIR_IMAGE_FILE)) {
            return $info.$this->_url_string($dir, DIR_IMAGE_FILE);
        }
        if (isset($images[0])) {
            return $info.$this->_url_string($dir, $images[0]);
        }
        ...
        if ($initial and file_exists(GALLERY_ROOT.ICONS_DIR.DIR_IMAGE_FILE)) {
            return $info.$this->_url_string(ICONS_DIR, DIR_IMAGE_FILE);
        }
        return $info;
    }
    ...
    private function _url_string($dir = '', $img = '') {
        $res = $dir.'*'.$img.'*';
    ...
    public function load_javascript() {
		...
		";
        echo "navLink[1] = '".$this->_url_string('')."';\n";
        ...
        if (GALLERY and is_array($links)) {
            ...
            for ($link_nr = $start_link; $link_nr < count($a_links); $link_nr++) {
                ...
                echo "navLink[".$i."] = '".$this->_url_string($a_links[$link_nr])."';\n";
                echo "navName[".$i."] = '".$this->_str_to_script($a_names[$link_nr])."';\n\n";
                $i++;
            }
            echo "dirLink[0] = '".$this->_url_string($a_links[count($a_links)-1])."';\n";
            ...
            echo "dirLink[0] = '".$this->_url_string("")."';\n";
        ...
        if ($this->haveAcess) {
            foreach ($dirs as $val) {
                ...
                echo "dirLink[" . ($item) . "] = '" . $this->_url_string((GALLERY . $val . "/")) . "';\n";
            ...
            foreach ($images as $val) {
                ...
                if ($showImage) {
                    ...
                    echo "imgLink[" . ($item) . "] = '" . $this->_url_string(GALLERY, $val) . "';\n";
        ...
        foreach ($files as $val) {
            $ext = $this->_ext($val);
            echo "fileLink[".($item)."] = '".$this->_url_string(GALLERY, $val)."';\n";
            if (FILE_THUMB_EXT and file_exists(GALLERY_ROOT.GALLERY.$val.FILE_THUMB_EXT)) {
                echo "fileThumb[".($item)."] = '".$this->_url_string(GALLERY, $val.FILE_THUMB_EXT)."';\n";
            } elseif (isset($file_ext_thumbs[$ext])) {
                echo "fileThumb[".($item)."] = '".$this->_url_string(ICONS_DIR, $file_ext_thumbs[$ext])."';\n";
            } else {
                echo "fileThumb[".($item)."] = '';\n";
            }
